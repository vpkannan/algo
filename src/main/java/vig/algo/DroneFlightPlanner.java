...
class DroneFlightPlanner {
  ...
  static int calcDroneMinEnergy(int[][] route) {
    ...
    int initialPosition = route[0][2];
    int maxHeight = initialPosition;
    for(int[] currentPosition : route) {
      maxHeight = (maxHeight < currentPosition[2]) ? currentPosition[2] : maxHeight;
    }
    return Math.abs(initialPosition - maxHeight);
  }
